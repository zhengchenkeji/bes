<template>
  ...
  >
    <span v-show="isActive()" class="iconfont icon-shuaxin" @mousedown="handleRotate"></span>
...
<script>
  ...
  import calculateComponentPositonAndSize from '@/utils/designer/calculateComponentPositonAndSize'
  import {mod360} from '@/utils/designer/translate'
  import {isPreventDrop} from '@/utils/designer/utils'
  ...
  export default {
    ...
    methods: {
      ...
      handleMouseDownOnShape(e) {
        ...
        let data = []
        if (this.element.component != null && this.element.component.indexOf('Light') > -1) {
          //如果是light更新第三方协议树复选框
          data.push(this.$store.state.otherDeviceTreeData)
          data = forEachChildren(data,'light');
          this.$store.commit('setOtherDeviceTree', data[0])
        }else if(this.element.component != null && this.element.component.indexOf('Button') > -1){

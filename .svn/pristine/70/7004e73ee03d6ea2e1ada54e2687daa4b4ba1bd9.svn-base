  ...
  <div class="app-container">
    ...
    <el-tabs v-model="activeName" type="border-card" style="height: 80.4vh;">
      <el-tab-pane label="数据项定义" name="first">
        ...
        <el-table v-loading="loading" :data="productItemDataList" height="550px" :header-cell-style="{background:'#FAFAFA'}">
          ...
          <el-table-column label="读写类型" align="center" prop="readType">
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.readType == '2'">只写</span>
            </template>
          </el-table-column>
          ...
          <div class="box">
            <el-form ref="form" :model="form" :rules="rules" label-width="100px">
              ...
              <el-form-item label="读写类型" prop="readType">
                <el-radio-group v-model="form.readType" @change="changeReadType">
                  <el-radio label="0" style="width: 50px">只读</el-radio>
                  <el-radio label="2" style="width: 50px">只写</el-radio>
                  <el-radio label="1" style="width: 50px">读写</el-radio>
                </el-radio-group>
              </el-form-item>
              <el-form-item label="读功能码" prop="functionCode" v-if="form.readType == '0' || form.readType == '1'">
                <el-input type="number" v-model="form.functionCode" placeholder="请输入功能码"/>
              </el-form-item>
              <el-form-item label="写功能码" prop="writeFunctionCode" v-if="form.readType == '1' || form.readType == '2'">
                <el-input type="number" v-model="form.writeFunctionCode" placeholder="请输入功能码"/>
              </el-form-item>
              ...
              <!-- 非高低位 结构体 枚举数据类型 -->
              <!-- <div v-if="form.highLow != '1'&& !showEnum && !showStruct" > -->
              <el-form-item label="寄存器数量" prop="registersNum">
                <el-input type="number" v-model="form.registersNum" placeholder="请输入需读取的寄存器数量"/>
              </el-form-item>
              <!-- </div> -->
        ...
        <el-dialog :title="titleTwo" :visible.sync="openTwo" width="520px" append-to-body>
          <div class="box">
            <el-form ref="formTwo" :model="formTwo" :rules="rulesTwo" label-width="100px">
              ...
              <!--              <el-form-item label="读写类型" prop="readType">-->
              <!--                <el-radio-group v-model="formTwo.readType" @change="changeUpdate">-->
              <!--                  <el-radio label="0" style="width: 140px">只读</el-radio>-->
              <!--                  <el-radio label="2" style="width: 140px">只写</el-radio>-->
              <!--                  <el-radio label="1" style="width: 140px">读写</el-radio>-->
              <!--                </el-radio-group>-->
              <!--              </el-form-item>-->
              <!--              <el-form-item label="读功能码" prop="functionCode" v-if="formTwo.readType == '0' || formTwo.readType == '1'">-->
              <!--                <el-input type="number" v-model="formTwo.functionCode" placeholder="请输入功能码"/>-->
              <!--              </el-form-item>-->
              <!--              <el-form-item label="写功能码" prop="writeFunctionCode" v-if="formTwo.readType == '1'">-->
              <!--                <el-input type="number" v-model="formTwo.writeFunctionCode" placeholder="请输入功能码"/>-->
              <!--              </el-form-item>-->
      ...
      <el-tab-pane label="功能定义" name="second">
        ...
        <el-table v-loading="loadingFunction" height="550px" :data="productFunctionList"
        ...
        <pagination
          ...
          @pagination="getFunctionList"
        />
        <!-- 添加或修改产品定义对话框 -->
        <el-dialog :title="titleFunction" :visible.sync="openFunction" width="500px" append-to-body>
          <el-form ref="formFunction" :model="formFunction" :rules="rulesFunction" label-width="100px">
            ...
            <el-form-item label="功能类型" prop="type">
              <el-radio-group v-model="formFunction.type" @change="changeType">
                <el-radio label="0" style="width: 150px">控制</el-radio>
                <el-radio label="1" style="width: 150px">采集</el-radio>
              </el-radio-group>
            </el-form-item>
            ...
            <el-form-item label="是否校检" prop="proofreadingState" v-if="formFunction.issuedType == '0'">
              <el-radio-group v-model="formFunction.proofreadingState">
                <el-radio label="0" style="width: 150px">否</el-radio>
                <el-radio label="1" style="width: 150px">是</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="指令" prop="instruct" v-if="formFunction.issuedType == '0'">
              <span slot="label">
                <el-tooltip content="*(固定开头)10(功能码)000c(寄存器起始地址)0001(寄存器数量)02(数据字节数)0000(数据)" placement="bottom"
                            effect="light">
            ...
            <el-form-item label="寄存器数量" prop="registerNumber"
                          v-if="formFunction.issuedType == '1' && formFunction.type=='1'">
              <el-input type="number" v-model="formFunction.registerNumber" placeholder="请输入寄存器数量"/>
            </el-form-item>
            <el-form-item label="下发值" prop="itemValue" v-if="showInt && formFunction.type=='0'">
              <el-input v-model="formFunction.itemValue" placeholder="请输入下发值"/>
            </el-form-item>
            <el-form-item label="下发值" prop="itemValue" v-if="showSelect && formFunction.type=='0'">
            ...
            <el-form-item :label="item.name" v-for="(item, index) in productItemParamsList"
                          v-if="formFunction.issuedType == '1' && formFunction.type=='0'"
                          :key="item.dataItem+'productItemParamsList'+index">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          writeFunctionCode: [{required: true, message: '请输入功能码', trigger: 'blur'}, {
            pattern: /^\+?[0-9][0-9]*$/,
            message: "只能输入数字",
          }],
          ...
          registersNum: [{required: true, message: '请输入需读取的寄存器数量', trigger: 'blur'}],
        ...
        rulesFunction: {
          ...
          instruct: [{required: true, message: '请输入指令', trigger: 'blur'}, {
            ...
            pattern: /^[*][A-Fa-f0-9YMDHmSW]+$/,
            message: "只能输入以*开头的16进制字母/数字",
          },],
          registerNumber: [{required: true, message: '请输入寄存器数量', trigger: 'blur'},
            {
              pattern: /^\+?[1-9][0-9]*$/,
              message: "只能输入正整数",
            },],
          dataItem: [{required: true, message: '请选择数据项', trigger: 'change'}],
        },
        rulesTwo: {
          productName: [{required: true, message: '请输入所属产品', trigger: 'blur'}],
          dataItemNum: [{required: true, message: '请输入数据项编号', trigger: 'blur'}],
    ...
    methods: {
      ...
      delGoodsInfo(index) {
        let that = this
        this.$modal.confirm('是否确认删除该枚举参数？').then(function () {
          that.form.enumDetail.splice(index, 1)
        }).then(() => {
        }).catch(() => {
      ...
      delGoodsInfoTwo(index) {
        let that = this
        this.$modal.confirm('是否确认删除该枚举参数？').then(function () {
          that.formTwo.enumDetail.splice(index, 1)
        }).then(() => {
        }).catch(() => {
          that.$modal.msgError("删除失败");
        });
      ...
      delHighInfo(index) {
        let that = this
        this.$modal.confirm('是否确认删除该高位参数？').then(function () {
          that.form.highDetail.splice(index, 1)
        }).then(() => {
        }).catch(() => {
          that.$modal.msgError("删除失败");
        });
      ...
      delLowInfo(index) {
        let that = this
        this.$modal.confirm('是否确认删除该低位参数？').then(function () {
          that.form.lowDetail.splice(index, 1)
        }).then(() => {
        }).catch(() => {
          that.$modal.msgError("删除失败");
        });
      ...
      delStructInfo(index) {
        let that = this
        this.$modal.confirm('是否确认删除该结构体参数？').then(function () {
          that.form.structDetail.splice(index, 1)
        }).then(() => {
        }).catch(() => {
          that.$modal.msgError("删除失败");
        });
      ...
      changeReadType(value) {
        if (value == '0') {
          this.form.writeFunctionCode = null
        } else if (value == '2') {
          this.form.functionCode = null
        }
      },
      //点击开启高低位
      changeStruct(value) {
      ...
      handleUpdate(row) {
        ...
        getProductItemData(id).then(response => {
          ...
          // response.data.sortNum = response.data.sortNum.toString(16)
                ...
                this.productItemParamsList.forEach((item, index) => {
                  if (item.selectList != null && item.selectList != undefined && item.selectList.length > 0) {
                    item.selectList.forEach(iteam => {
                      if (response1.data.paramsList != null && response1.data.paramsList != undefined && response1.data.paramsList.length > 0) {
      ...
      submitForm() {
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            //如果有高低位或者结构体参数，则直接清空  单位，是否报存，能源类型，小数点位置，比率
            if ((this.form.highDetail != null && this.form.highDetail != undefined && this.form.highDetail.length > 0) ||
              (this.form.lowDetail != null && this.form.lowDetail != undefined && this.form.lowDetail.length > 0) ||
              (this.form.structDetail != null && this.form.structDetail != undefined && this.form.structDetail.length > 0)) {
              this.form.preserveType = null;
              this.form.radixPoint = null;
              this.form.energyCode = null;
              this.form.ratioSize = null;
            }
            if (this.form.structDetail != null && this.form.structDetail != undefined && this.form.structDetail.length > 0) {
              this.form.dataUnit = null;
            }
      ...
      submitFormFunction() {
        ...
        this.$refs["formFunction"].validate(valid => {
          if (valid) {
            //采集清空参数
            if (this.formFunction.type != null && this.formFunction.type != undefined && this.formFunction.type == '1') {
              this.formFunction.paramsList = []
              this.formFunction.itemValue = null
            } else if (this.formFunction.type != null && this.formFunction.type != undefined && this.formFunction.type == '0') {
              this.formFunction.registerNumber = null
            }
            //指令下发
            if (this.formFunction.issuedType != null && this.formFunction.issuedType != undefined && this.formFunction.issuedType == '1') {
              this.formFunction.instruct = null
            } else if (this.formFunction.issuedType != null && this.formFunction.issuedType != undefined && this.formFunction.issuedType == '0') {
              this.formFunction.paramsList = []
              this.formFunction.dataItem = null
            }
      ...
      changeIssuedType(value) {
        ...
        if (this.formFunction.dataItem != null && this.formFunction.dataItem != undefined && this.formFunction.dataItem != '') {
          const id = this.formFunction.dataItem
          this.getParamsListByItemDataId(id)
        }
      },
      changeType(value) {
        if (value == '0') {
        ...
        if (this.productName == null || this.productName == undefined) {
          getProduct(this.productId).then(response => {
            ...
            listProductItemData(this.queryParams).then(response => {
              this.productItemDataList = response.data;
      ...
      getFunctionList() {
        ...
        listProductFunction(this.queryParamsFunction).then(response => {
          this.productFunctionList = response.data;

  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="jobList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="点位" width="100" align="center" prop="id">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-view" @click="getPointList(scope.row)">查看点位</el-button>
        </template>
      </el-table-column>
    ...
    <el-drawer size='28%' :title="drawertitle" :visible.sync="visible" direction="rtl">
      <div style="height: 700px; overflow: auto;">
        <el-tree :data="treedata" :props="props" show-checkbox node-key="id" :default-checked-keys="treeids"
          :default-expanded-keys="expandedKeys" ref="tree" @check="handleCheckChange">
        </el-tree>
      </div>
    ...
    <el-dialog :title="title" :visible.sync="open" :close-on-click-modal="false" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row>
          <el-col :span="12">
            <el-form-item label="任务名称" prop="jobName">
              <el-input style="width: 33.3vw" v-model="form.jobName" placeholder="请输入任务名称" />
            </el-form-item>
          </el-col>
          <!--          <el-col :span="12">-->
    ...
    <el-dialog title="点位" :visible.sync="pointTableVisible">
      ...
      <pagination v-show="PointTotal > 0" :total="PointTotal" :page.sync="pointqueryParams.pageNum"
        :limit.sync="pointqueryParams.pageSize" @pagination="pointListData" />
...
<script>
import { getDeviceTreee, listDevicejob, addDevicejob, updateJob, getPointList, getDeviceJob, getChcekNodes, delDeviceJob, changeJobStatus, runJob, allEquipmentFunctionTree } from "@/api/monitor/timedObtainEnergy";
import Crontab from '@/components/Crontab'
import { JobSettings } from '@/api/monitor/JobSetting'
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        ...
        pageSize: 10,
        jobName: undefined,
        jobGroup: JobSettings.energyJob,
        status: undefined,
        remark: ""
      },
  ...
  methods: {
    ...
    getDeviceTreeList() {
      allEquipmentFunctionTree().then(response => {
        if (response.code == 200) {
          this.treedata = this.handleTree(response.data, "id", "pid");
        }
      })
    },
    ...
    handleCheckChange(data, checked, indeterminate) {
      ...
      // 如果未给当前选中的控制器赋值 则赋值 或未选中子节点
      if (!this.controllerId || this.$refs.tree.getCheckedKeys().length == 1) {
        this.controllerId = data.controllerId
      }
      //  如果不对应则取消
      if (data.controllerId != this.controllerId) {
        this.$modal.msgWarning("不允许跨控制器选择功能！");
    ...
    handleJobLog(row) {
      ...
      const jobGroup = JobSettings.energyJob;
    ...
    handleUpdate(row) {
      ...
      getDeviceJob(jobId).then(response => {
        ...
        for (let i = 0; i < this.treeids.length; i++) {
          const arrayKeys = this.getparentlist(this.treeids[i], this.treedata);
          console.log(this.treeids[i], arrayKeys)
    ...
    submitForm: function () {
      ...
      this.$set(this.form, 'jobGroup', JobSettings.energyJob);

  ...
  props: {
    ...
    treeNodeMsgs: {
      ...
      default: {
        ...
        description: null,//描述
        collectPeriod: null,//采集周期： 分钟（只有能耗采集器有）
        uploadPeriod: null,//上传周期：分钟（只有能耗采集器有）
        savePeriod: 0,//保存周期：小时
      }
    },
    deviceNodeCode: {
      type: String
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        ip: [
          {required: true,message: "ip格式不正确", trigger: "blur", pattern: /((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3}/}
        ],
        gateWay: [
          {required: true,message: "ip格式不正确", trigger: "blur", pattern: /((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3}/}
        ],
  ...
  methods: {
    ...
    reset() {
      this.form = {
        ...
        active: 1,//使能状态  0：不使能、1：使能
        description: '',//描述
        collectPeriod: this.collectOruploadOptions.length>0 ? this.collectOruploadOptions[0].value : null,//采集周期： 分钟（只有能耗采集器有）
        uploadPeriod: this.collectOruploadOptions.length>0 ? this.collectOruploadOptions[0].value : null,//上传周期：分钟（只有能耗采集器有）
        savePeriod: this.savePeriodOptions[0].value,//保存周期
      };
      // this.resetForm("form");
    },
    //生成保存周期/上传周期/采集周期的数据
